...
export class InternetProvider extends BaseProvider {
  ...
  ascii_company_email(): string {
   ...
   uri_path(deep?: number): string {
     const d = deep ?? this.randomInt(1, 3);
     const parts: string[] = [];
     for (let i = 0; i < d; i++) {
   ...
   slug(value?: string): string {
     if (!value) {
       value = this.lorem?.()?.text?.(20) || "hello world";
     }
     return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
   }

   image_url(width?: number, height?: number, placeholder_url?: string): string {
     const w = width ?? this.randomInt(0, 1024);
     const h = height ?? this.randomInt(0, 1024);
   ...
   iana_id(): string {
     return String(this.randomInt(1, 8888888));
   }

   ripe_id(): string {
     const lex = "?".repeat(this.randomInt(2, 4));
     const num = "%".repeat(this.randomInt(1, 5));
     return this.bothify(`ORG-${lex}${num}-RIPE`).toUpperCase();
   }
   ...
   nic_handle(suffix = "FAKE"): string {
     ...
     const lex = "?".repeat(this.randomInt(2, 4));
     const num = "%".repeat(this.randomInt(1, 5));
     return this.bothify(`${lex}${num}-${suffix}`).toUpperCase();
   }

    nic_handles(count = 1, suffix = "FAKE"): string[] {
   ...
   dga(year?: number, month?: number, day?: number, tld?: string, length?: number): string {
     const y = year ?? this.randomInt(1, 9999);
     const m = month ?? this.randomInt(1, 12);
     const d = day ?? this.randomInt(1, 30);
     const t = tld ?? this.tld();
     const len = length ?? this.randomInt(2, 63);
     let domain = "";
     let yr = y, mo = m, dy = d;
     for (let i = 0; i < len; i++) {
   ...
   ipv4(network = false, address_class?: string | null, private_?: boolean | null): string {
     ...
     const getOctet = () => this.randomInt(0, 255);
     if (private_ === true) {
       // Private ranges: 10.x.x.x, 172.16-31.x.x, 192.168.x.x
       const cls = this.randomInt(0, 2);
       if (cls === 0) return `10.${getOctet()}.${getOctet()}.${getOctet()}`;
       if (cls === 1) return `172.${this.randomInt(16, 31)}.${getOctet()}.${getOctet()}`;
       return `192.168.${getOctet()}.${getOctet()}`;
     }
     if (private_ === false) {
       // Public IP (avoid private ranges)
       let first = this.randomInt(1, 223);
   ...
   ipv6(network = false): string {
     const parts: string[] = [];
     for (let i = 0; i < 8; i++) {
       parts.push(this.randomInt(0, 0xffff).toString(16));
     }
     let addr = parts.join(":");
     if (network) {
       const prefix = this.randomInt(0, 128);
       addr += `/${prefix}`;
     }
     return addr;
   }
    ...
    http_method(): string {
      return this.randomElement(["GET", "HEAD", "POST", "PUT", "DELETE", "CONNECT", "OPTIONS", "TRACE", "PATCH"]);
    }

    port_number(system = false, user = false, dynamic = false): number {
      if (system) return this.randomInt(0, 1023);
      if (user) return this.randomInt(1024, 49151);
      if (dynamic) return this.randomInt(49152, 65535);
      return this.randomInt(0, 65535);
    }

    mac_address(multicast = false): string {
     const mac: number[] = [];
     for (let i = 0; i < 5; i++) mac.push(this.randomInt(0x00, 0xff));
     if (multicast) mac.unshift(this.randomInt(0x01, 0xff) | 1);
     else mac.unshift(this.randomInt(0x00, 0xfe) & 0xfe);
